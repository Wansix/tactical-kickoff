...
type MatchReport={seed:number;goals:number;blueGoals:number;orangeGoals:number;kicks:number;wallBounces:number;signature:string;maxCollisionRun:number;maxDirectionReversalRun:number;robotRanges:Record<string,{x:number;y:number}>};
...
function run(seed:number):MatchReport{
  ...
  return {seed,goals:goals.length,blueGoals,orangeGoals,kicks:events.filter(e=>e.type==='kick').length,wallBounces:events.filter(e=>e.type==='wall-bounce').length,signature,maxCollisionRun:collisionRun(events),maxDirectionReversalRun:reversalRun(frames),robotRanges:rangeReport(frames)};
}
...
const result={matches:reports.length,uniqueSignatures:signatures.size,totalGoals,blueGoals,orangeGoals,scoringSeedBlue,scoringSeedOrange,maxCollisionRun,maxDirectionReversalRun,scoringTeams:[...scoringTeams],failures};
