...
const lab=new TestLab(document.querySelector('aside')!,config=>scene.configureLab(config.blueBrain,config.blueBody,config.orangeBrain,config.orangeBody,config.opponentEnabled,config.blueRoster,config.orangeRoster,config.bluePlacement,config.orangePlacement,config.seedEnabled,config.seed),active=>scene.setLabMode(active),()=>scene.start());
scene.setLabRobotMoveHandler((team,index,x,y)=>lab.handleFieldRobotMove(team,index,x,y));
scene.setLabRobotSelectHandler((team,index)=>lab.selectFieldRobot(team,index));
...
if(import.meta.env.DEV){
  ...
  (window as Window & {__tacticalKickoffQA?:{configureSweeper:()=>void;toggleDebug:()=>boolean;inspect:()=>unknown[];getState:()=>unknown;getTelemetry:()=>unknown[];getLabSelectionMetrics:()=>unknown}}).__tacticalKickoffQA={configureSweeper,toggleDebug:()=>scene.toggleDebug(),inspect:()=>scene.inspect(),getState:()=>scene.getState(),getTelemetry:()=>scene.getTelemetry(),getLabSelectionMetrics:()=>scene.getLabSelectionMetrics()};
}
