...
const bindControls=(scene:GameScene)=>{
  ...
  document.querySelectorAll<HTMLElement>('.team-config').forEach((card)=>{
    ...
    slotRows.forEach((row)=>{
      ...
      slots.forEach((slot)=>slot.onclick=()=>{slots.forEach(candidate=>candidate.classList.remove('selected'));slot.classList.add('selected');});
    });
    card.querySelector<HTMLButtonElement>('.apply-roster')!.onclick=()=>{
      const team=card.dataset.team as 'blue'|'orange';
      const types=Array.from(card.querySelectorAll<HTMLSelectElement>('.robot-type')).map(select=>select.value) as ['striker','striker'];
      ...
      scene.configureRoster(team,types,selected);
    };
  });
