...
app.innerHTML=`<header><div><span class="kicker">ROBO CUP / MOBILE SIMULATION LAB</span><h1>TACTICAL KICKOFF</h1></div><div class="legend"><span class="blue-dot"></span> BLUE ↑ <span class="orange-dot"></span> ORANGE ↓</div></header><main><div id="game"></div><aside><div class="panel"><h2>MATCH CONTROL</h2><button id="start">▶ START MATCH</button><button id="pause">Ⅱ PAUSE</button><button id="restart">↻ RESTART</button><label>SIMULATION SPEED <select id="speed"><option value="0.5">0.5×</option><option value="1" selected>1×</option><option value="2">2×</option><option value="4">4×</option></select></label></div><div class="panel"><h2>COMPOSITION</h2><p class="hint">Swap the blue unit's duties after the whistle. Every team stays 2v2 — no keeper.</p><button id="swap-blue">⇄ SWAP BLUE ROLES</button><button id="replay">⟲ REPLAY MATCH</button></div><div class="panel telemetry"><h2>LIVE TELEMETRY</h2><p><b>STRIKER</b><br>presses ball carrier, attacks goal</p><p><b>ANCHOR</b><br>covers lane, stabilizes shape</p><small>SEE SHAPES + ACTION TAGS ON FIELD<br>DETERMINISTIC SEED / 2025</small></div></aside></main><footer>90 SECOND MOBILE WATCH MODE <span>•</span> ENGINE-INDEPENDENT MATCH LOGIC <span>•</span> PHASER 3 PRESENTATION</footer>`;
