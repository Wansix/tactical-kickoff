...
if(import.meta.env.DEV){
  (window as Window & {__tacticalKickoffQA?:{configureStriker1v1:()=>void;forceGoal:()=>void;forceGoalHold:()=>void;advance:(seconds:number)=>void;forceKick:()=>void;toggleDebug:()=>boolean;inspect:()=>unknown[];getState:()=>unknown;getTelemetry:()=>unknown[]}}).__tacticalKickoffQA={configureStriker1v1:()=>scene.configureStriker1v1Test(),forceGoal:()=>scene.forceGoalForTest(),forceGoalHold:()=>scene.forceGoalForTest(),advance:(seconds:number)=>scene.advanceForTest(seconds),forceKick:()=>scene.forceKickForTest(),toggleDebug:()=>scene.toggleDebug(),inspect:()=>scene.inspect(),getState:()=>scene.getState(),getTelemetry:()=>scene.getTelemetry()};
}
