...
export class GameScene extends Phaser.Scene {
  ...
  private render():void { const s=this.sim.state;this.scoreText.setText(`BLUE  ${s.score.blue}   —   ${s.score.orange}  ORANGE`);const remain=Math.ceil(90-s.elapsed);this.timeText.setText(`${Math.floor(remain/60).toString().padStart(2,'0')}:${(remain%60).toString().padStart(2,'0')}`);this.statusText.setText(`${s.status.toUpperCase()}  //  ${this.speed.toFixed(1)}x  //  BLUE ↑  ORANGE ↓`);this.ball.setPosition(this.field.x+s.ball.x,this.field.y+s.ball.y);for(const r of s.robots){const c=this.robotGraphics.get(r.id);if(c){c.setPosition(this.field.x+r.x,this.field.y+r.y);const label=c.list[2] as Phaser.GameObjects.Text;label.setText(`${r.role.toUpperCase()}\n${r.action}`);}}}
}
