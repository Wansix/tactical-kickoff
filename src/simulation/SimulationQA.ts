...
export function detectAnomalies(run:ScenarioRun,field={width:540,height:860},windowTicks=120):SimulationAnomaly[]{
  ...
  for(const frame of frames){
    ...
    const goalNetDepth=(frame.goalResetTimer>0||frame.safetyGoalPending!==undefined||frame.kickoffSafetyTimer>0&&((frame.ball.y<0&&frame.ball.x>=GOAL_GEOMETRY.mouthLeft&&frame.ball.x<=GOAL_GEOMETRY.mouthRight)||(frame.ball.y>field.height&&frame.ball.x>=GOAL_GEOMETRY.mouthLeft&&frame.ball.x<=GOAL_GEOMETRY.mouthRight)))?GOAL_GEOMETRY.depth:60;
  ...
  // Index each actor once. The previous implementation repeatedly scanned and sliced
  // every frame for every actor, making the 100-seed/60-second gate effectively
  // quadratic in telemetry size and causing a false QA timeout without changing any
  // gameplay result.
  const robotsById=new Map<string,NonNullable<TelemetryFrame['robots'][number]>[]>();
  for(const id of ids) robotsById.set(id,frames.map(frame=>frame.robots.find(robot=>robot.id===id)).filter(Boolean) as NonNullable<TelemetryFrame['robots'][number]>[]);
  const recoveryTicks=run.events.filter(event=>event.type==='stuck-recovery').map(event=>event.tick).sort((a,b)=>a-b);
  const recoveryNear=(tick:number)=>{let low=0,high=recoveryTicks.length;while(low<high){const mid=(low+high)>>1;if(recoveryTicks[mid]<tick-180)low=mid+1;else high=mid;}return low<recoveryTicks.length&&recoveryTicks[low]<=tick+180;};
  for(const id of ids){
    const rows=robotsById.get(id)??[];
    for(let end=windowTicks;end<=rows.length;end++){
      const window=rows.slice(end-windowTicks,end);
      ...
      if(width<10&&height<10&&averageSpeed<5&&active&&currentRole!=='goalkeeper'&&!['COVER','RESET','KICK'].includes(currentAction??'')&&!((currentRole==='bulwark'||currentRole==='sweeper')&&currentAction==='PRESS')&&!recoveryNear(last.tick)){anomalies.push({kind:'local-stuck',tick:last.tick,elapsed:last.elapsed,robotId:id,message:`robot stayed within ${Math.max(width,height).toFixed(1)}px at ${averageSpeed.toFixed(1)}px/s for ${windowTicks} ticks`,details:{width,height,averageSpeed,action:currentAction}});break;}
    }
  }
  for(const id of ids){
    const rows=robotsById.get(id)??[]; let previousAction:Action|undefined;let stateRun=0;let targetChanges=0;let rollingSpeed=0;
    for(let index=0;index<rows.length;index++){
      const robot=rows[index]; const previous=rows[index-1];
      if(robot.action===previousAction)stateRun++;else stateRun=1; previousAction=robot.action;
      if(previous&&Math.hypot(robot.moveTargetX-previous.moveTargetX,robot.moveTargetY-previous.moveTargetY)>18)targetChanges++;else targetChanges=0;
      rollingSpeed+=Math.hypot(robot.vx,robot.vy); if(index>=60)rollingSpeed-=Math.hypot(rows[index-60].vx,rows[index-60].vy);
      const sampleCount=Math.min(index+1,60); const averageSpeed=rollingSpeed/sampleCount;
      if(targetChanges>=30&&averageSpeed<5&&!recoveryNear(frames[index].tick)&&!((robot.archetype==='bulwark'||robot.archetype==='sweeper')&&['PRESS','COVER','RESET','KICK'].includes(robot.action)))anomalies.push({kind:'target-thrash',tick:frames[index].tick,elapsed:frames[index].elapsed,robotId:id,message:'move target changed on 30 consecutive ticks while nearly stationary',details:{targetChanges,averageSpeed,action:robot.action}});
      if(stateRun>=windowTicks*3&&['CARRY','PRESS'].includes(robot.action)&&averageSpeed<5&&!((robot.archetype==='bulwark'||robot.archetype==='sweeper')&&robot.action==='PRESS')&&!recoveryNear(frames[index].tick)){anomalies.push({kind:'state-stuck',tick:frames[index].tick,elapsed:frames[index].elapsed,robotId:id,message:`state ${robot.action} persisted for ${stateRun} ticks while nearly stationary`,details:{state:robot.action,run:stateRun,averageSpeed}});break;}
    }
  }
