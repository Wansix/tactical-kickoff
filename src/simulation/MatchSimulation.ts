...
export type SweeperState = 'HOLD_ZONE' | 'INTERCEPT_STAGE' | 'INTERCEPT' | 'CLEAR' | 'RETURN_TO_ZONE' | 'HOLD_POST' | 'CLEAR_KICK' | 'RETURN_TO_POST';
export interface DefensiveZone { left:number; top:number; right:number; bottom:number; }

type MatchStatus = 'ready' | 'running' | 'paused' | 'finished';
export type EventType = 'robot-ball-collision' | 'robot-robot-collision' | 'kick' | 'goal' | 'kickoff' | 'wall-bounce' | 'stuck-recovery' | 'state-change' | 'target-change' | 'decision' | 'ai-warning' | 'goal-area-exit-damping';
...
export interface Robot {
  ...
  defensiveZone?:DefensiveZone; sweeperState:SweeperState; backpedal:boolean; interceptReason:string; clearImpulse:number; returnTick:number; clearCooldown:number;
...
export interface SimulationEvent {
  ...
  candidate?:string; reason?:string; causeContactTick?:number; boundary?:number; speedBefore?:number; speedAfter?:number; crossingY?:number;
...
export interface TelemetryFrame {
  tick:number; elapsed:number; status:MatchStatus; goalResetTimer:number; kickoffSafetyTimer:number; safetyGoalPending?:Team; score:Record<Team,number>;
  ball:{x:number;y:number;vx:number;vy:number};
  robots:Array<{id:string;team:Team;role:Role;archetype:RobotArchetype;x:number;y:number;vx:number;vy:number;maxSpeed:number;facingX:number;facingY:number;kickCooldown:number;kickLockout:number;chargePauseTicks:number;target:string;action:Action;moveTargetX:number;moveTargetY:number;distanceToBall:number;distanceToTarget:number;sweeperState:SweeperState; defensiveZone?:DefensiveZone; backpedal:boolean;interceptReason:string;clearImpulse:number;returnTick:number;clearCooldown:number;lastDecisionReason:string;stateChangedAt:number;lastKickAt?:number;kickTarget?:string;kickDirectionX?:number;kickDirectionY?:number;kickPower?:number}>;
...
export interface SimulationOptions { seedEnabled?:boolean; }
...
export const SWEEPER_ZONE_SIZE=190;
const SWEEPER_ZONE_HALF=SWEEPER_ZONE_SIZE/2;
const SWEEPER_ZONE_HYSTERESIS=18;
const SWEEPER_INTERCEPT_COOLDOWN_TICKS=24;
...
const GOALKEEPER_CLEAR_SPEED=1040;
const SWEEPER_EXIT_SPEED=360;
export const SWEEPER_FORWARD_LIMIT=180;
const GOAL_AREA_BALL_SPEED_CAP=180;
const GOAL_AREA_BALL_DAMPING=0.72;
const CANNON_KICK_POWER=315;
const ROBOT_SPEED_MULT=2;
const ROBOT_ACCEL_MULT=2;

export class MatchSimulation {
  readonly field={width:540,height:860}; readonly duration=90; readonly seed:number; readonly seedEnabled:boolean; private paused=false; private accumulator=0; private tickIndex=0;
  private kickoffTimer=0; private kickoffRaceTicks=0; private kickoffSafetyTimer=0; private safetyGoalPending:Team|undefined; private initialKickoffSafety=true; private kickoffPreferredTeam:Team='blue'; private kickoffFirstKickPending=false; private kickDebugLine=false; private ballKickInvuln=0; private ballContactCooldown:Record<string,number>={}; private robotCollisionCooldown:Record<string,number>={}; private lastRobotBallContactTick:Record<string,number>={}; private ballStuckTicks=0; private cornerStuckTicks=0; private sideWallStuckTicks=0; private sideWallRecoveryLatched=false; private stuckRecoveryCooldown=0; private cornerRecoveryCooldown=0; private centralDeflectionCooldown=0; private goalTeam:Team|undefined; private sweeperClearTeam:Team|undefined; private sweeperClearNeedsExit=false; private goalAreaDampingTicks=0; private wallContact={left:false,right:false,top:false,bottom:false}; private lastBallX=270; private lastBallY=430; private lastKickTeam:Team|undefined; private lastKickX=0; private lastKickY=0; private lastKickElapsed=-10; private kickBurstCount=0; private kickBurstStart=-10; private events:SimulationEvent[]=[]; private telemetry:TelemetryFrame[]=[];
  private readonly seedFormation:Record<string,{x:number;y:number}>;
  state:MatchState;

  constructor(seed=42,composition?:Partial<TeamComposition>,options:SimulationOptions={seedEnabled:true}){
    this.seedEnabled=options.seedEnabled??true;
    this.seed=this.seedEnabled?seed:Math.floor(Math.random()*0x7fffffff);

    const formationOffset=((this.seed*37)%121)-60;
    const blue=composition?.blue??['striker','bulwark'];
    const orange=composition?.orange??['striker','bulwark'];
    MatchSimulation.validateComposition({blue,orange});
    const roster=(team:Team, archetypes:RobotArchetype[])=>archetypes.map((archetype,index)=>{
      if(archetypes.length>=4){
        ...
        const columns=[110,270,430];
        const x=goalkeeper?this.field.width/2:columns[index%columns.length];
        const row=Math.floor(index/columns.length);
        const y=goalkeeper?(team==='blue'?this.field.height-GOALKEEPER_HOME_DEPTH:GOALKEEPER_HOME_DEPTH):(team==='blue'?this.field.height-150-row*150:150+row*150);
        const startSlot:StartSlot=goalkeeper?'goalkeeper':index%3===1?'center':index%3===0?'left':'right';
  ...
  private robot(team:Team,index:number,x:number,y:number,archetype:RobotArchetype,startSlot:StartSlot):Robot {
    ...
    return {id:`${team}-${index}`,team,role,shape:this.shapeForRole(role),archetype,x,y,startSlot,homeX:x,homeY:y,lastMoveX:0,lastMoveY:0,reversalRun:0,reversalLockTicks:0,chargePauseTicks:0,collisionEscapeTicks:0,collisionEscapeX:0,collisionEscapeY:0,vx:0,vy:0,facingX:0,facingY:team==='blue'?-1:1,radius:ROBOT_RADIUS,
      ...
      action:'RESET',target:'BALL',kickCooldown:0,kickLockout:0,moveTargetX:x,moveTargetY:y,distanceToBall:0,distanceToTarget:0,defensiveZone:archetype==='sweeper'?this.defaultSweeperZone(team):undefined,sweeperState:'HOLD_ZONE',backpedal:false,interceptReason:'initial formation',clearImpulse:0,returnTick:0,clearCooldown:0,lastDecisionReason:'initial formation',stateChangedAt:0};
  }
  ...
  private profileForArchetype(archetype:RobotArchetype){return ({striker:{mass:2,maxSpeed:230,acceleration:1000},bulwark:{mass:3.2,maxSpeed:156,acceleration:600},scout:{mass:1.6,maxSpeed:290,acceleration:1240},dribbler:{mass:2.1,maxSpeed:210,acceleration:920},cannon:{mass:2.5,maxSpeed:184,acceleration:760},sweeper:{mass:3,maxSpeed:260,acceleration:1500},goalkeeper:{mass:3.8,maxSpeed:200,acceleration:1300}} as const)[archetype];}
  private defaultSweeperZone(team:Team):DefensiveZone { const centerY=team==='blue'?585:275; return {left:this.field.width/2-SWEEPER_ZONE_HALF,top:centerY-SWEEPER_ZONE_HALF,right:this.field.width/2+SWEEPER_ZONE_HALF,bottom:centerY+SWEEPER_ZONE_HALF}; }

  start(){
    if(this.state.status==='ready') { this.paused=false; this.kickoffTimer=0.75; this.kickoffRaceTicks=180; this.kickoffSafetyTimer=5; this.kickoffFirstKickPending=false; this.state.status='running'; this.recordEvent({type:'kickoff',x:this.state.ball.x,y:this.state.ball.y}); }
  }
  ...
  setSweeperZone(robotId:string, zone:DefensiveZone):DefensiveZone {
    if(this.state.status!=='ready') throw new Error('sweeper zone is locked after match start');
    const robot=this.state.robots.find(candidate=>candidate.id===robotId && candidate.archetype==='sweeper');
    ...
    const ownCenterY=this.clamp(center.y,ownTop+height/2,ownBottom-height/2);
    const ownCenterX=this.clamp(center.x,28+width/2,this.field.width-28-width/2);
    robot.defensiveZone={left:ownCenterX-width/2,top:ownCenterY-height/2,right:ownCenterX+width/2,bottom:ownCenterY+height/2};
    robot.homeX=ownCenterX; robot.homeY=ownCenterY; this.seedFormation[robot.id]={x:ownCenterX,y:ownCenterY};
    return {...robot.defensiveZone};
  }
  ...
  private stepFixed(){
    ...
    for(const robot of this.state.robots){ robot.kickCooldown=Math.max(0,robot.kickCooldown-dt); robot.kickLockout=Math.max(0,robot.kickLockout-dt); robot.clearCooldown=Math.max(0,robot.clearCooldown-dt); robot.chargePauseTicks=Math.max(0,robot.chargePauseTicks-1); robot.clearImpulse=0; }
    ...
    for(const robot of this.state.robots){const minX=robot.archetype==='goalkeeper'?GOAL_AREA.left+ROBOT_RADIUS:28;const maxX=robot.archetype==='goalkeeper'?GOAL_AREA.right-ROBOT_RADIUS:this.field.width-28;const goalkeeperHomeY=robot.team==='blue'?this.field.height-GOALKEEPER_HOME_DEPTH:GOALKEEPER_HOME_DEPTH;const minY=robot.archetype==='goalkeeper'?(robot.team==='blue'?goalkeeperHomeY-GOALKEEPER_STEP_OUT:goalkeeperHomeY):28;const maxY=robot.archetype==='goalkeeper'?(robot.team==='blue'?goalkeeperHomeY:goalkeeperHomeY+GOALKEEPER_STEP_OUT):this.field.height-28;robot.x=this.clamp(robot.x,minX,maxX);robot.y=this.clamp(robot.y,minY,maxY); }
    this.resolveRobotBallCollisions();
    for(const robot of this.state.robots) if(robot.archetype==='sweeper'){const bounded=this.clampSweeperToZone(robot,robot.x,robot.y);if(bounded.x!==robot.x)robot.vx=0;if(bounded.y!==robot.y)robot.vy=0;robot.x=bounded.x;robot.y=bounded.y;}
  ...
  private decideAndMoveRobots(dt:number){
    ...
    for(const robot of orderedRobots){
      ...
      const seedBias=(((this.seed*37)%121)-60)*0.35*(robot.team==='blue'?1:-1);
      ...
      switch(robot.archetype){
        case 'goalkeeper': {
          ...
          const goalkeeperHomeY=robot.team==='blue'?this.field.height-GOALKEEPER_HOME_DEPTH:GOALKEEPER_HOME_DEPTH;
          const ballMovingTowardOwnGoal=robot.team==='blue'?b.vy>35:b.vy<-35;
          const nearPunchPosition=Math.hypot(b.x-robot.x,b.y-robot.y)<=GOALKEEPER_PUNCH_REACH+30;
          const stepOut=ownAreaNearBall&&ballMovingTowardOwnGoal&&nearPunchPosition?Math.min(GOALKEEPER_STEP_OUT,Math.max(0,Math.abs(b.y-goalkeeperHomeY))):0;
          targetX=this.clamp(centerX+(b.x-centerX)*0.45,GOAL_AREA.left+ROBOT_RADIUS,GOAL_AREA.right-ROBOT_RADIUS);
          targetY=ownAreaNearBall?goalkeeperHomeY+(robot.team==='blue'?-stepOut:stepOut):goalkeeperHomeY;
          robot.target='GOAL_LINE';action='COVER';break;
        }
        case 'striker': {
          // Direct charge with bounded lead: predict only briefly while the ball is away from a side wall.
          // Near a wall, current center is safer because reflections/contacts can invalidate the lead instantly.
          const nearSideWall=b.x<=70||b.x>=this.field.width-70;
          const lead=nearSideWall?0:0.10;
          targetX=this.clamp(b.x+b.vx*lead,28,this.field.width-28);
          targetY=this.clamp(b.y+b.vy*lead,28,this.field.height-28);
          action='PRESS';
          break;
        }
        case 'scout': {
        ...
        case 'sweeper': {
          const zone=robot.defensiveZone??this.defaultSweeperZone(robot.team);
          const homeX=(zone.left+zone.right)/2,homeY=(zone.top+zone.bottom)/2;
          const zoneContains=(margin:number)=>b.x>=zone.left-margin&&b.x<=zone.right+margin&&b.y>=zone.top-margin&&b.y<=zone.bottom+margin;
          const strictlyInsideZone=zoneContains(0);
          const zoneThreatWindow=zoneContains(SWEEPER_ZONE_HYSTERESIS);
          const distanceToBall=Math.hypot(b.x-robot.x,b.y-robot.y);
          const nearestZonePoint={x:this.clamp(b.x,zone.left,zone.right),y:this.clamp(b.y,zone.top,zone.bottom)};
          const pressureTarget= strictlyInsideZone ? {x:b.x,y:b.y} : nearestZonePoint;
          const activePressure=robot.sweeperState==='INTERCEPT_STAGE'||robot.sweeperState==='INTERCEPT'||robot.sweeperState==='RETURN_TO_ZONE';
          const threatWindow=activePressure?(zoneThreatWindow||distanceToBall<260):(strictlyInsideZone||distanceToBall<260);
          const threat=this.kickoffRaceTicks<=0&&threatWindow;
          if(robot.sweeperState==='HOLD_POST') robot.sweeperState='HOLD_ZONE';
          if(robot.sweeperState==='RETURN_TO_POST') robot.sweeperState='RETURN_TO_ZONE';
          if(robot.sweeperState==='CLEAR_KICK') robot.sweeperState='CLEAR';
          const previousState=robot.sweeperState;
          if(robot.sweeperState==='HOLD_ZONE'&&!threat) { /* hold */ }
          else if(robot.sweeperState==='HOLD_ZONE'&&threat) robot.sweeperState='INTERCEPT_STAGE';
          else if(robot.sweeperState==='RETURN_TO_ZONE'){
            const atHome=Math.hypot(robot.x-homeX,robot.y-homeY)<24&&Math.hypot(robot.vx,robot.vy)<80;
            if(robot.clearCooldown<=0&&threat&&this.tickIndex-robot.returnTick>=SWEEPER_INTERCEPT_COOLDOWN_TICKS) robot.sweeperState='INTERCEPT_STAGE';
            else if(!threat&&atHome) robot.sweeperState='HOLD_ZONE';
          }
          else if((robot.sweeperState==='INTERCEPT_STAGE'||robot.sweeperState==='INTERCEPT')&&!threat) robot.sweeperState='RETURN_TO_ZONE';
          else if(robot.sweeperState==='INTERCEPT_STAGE'&&distanceToBall<220) robot.sweeperState='INTERCEPT';
          else if(robot.sweeperState==='CLEAR') robot.sweeperState='RETURN_TO_ZONE';
          if(robot.sweeperState==='RETURN_TO_ZONE'&&threat&&previousState!=='CLEAR'&&robot.clearCooldown<=0) robot.sweeperState='INTERCEPT';
          if(previousState!==robot.sweeperState){
            ...
            if(robot.sweeperState==='RETURN_TO_ZONE') robot.returnTick=this.tickIndex;
          ...
          if(robot.sweeperState==='INTERCEPT_STAGE'||robot.sweeperState==='INTERCEPT'){
            const urgent=distanceToBall<220;
            const directTarget=pressureTarget;
            const goalThreat=this.isInsideOwnGoalArea(robot.team,b.x,b.y)&&((robot.team==='blue'?b.vy>60:b.vy< -60)||urgent);
            const angleTarget=this.goalAngleBlockTarget(robot.team,directTarget.x,directTarget.y);
            const blockTarget=goalThreat?this.clampSweeperRoleTarget(robot.team,angleTarget.x,angleTarget.y):directTarget;
            const zoneTarget=this.clampSweeperToZone(robot,blockTarget.x,blockTarget.y);
            targetX=zoneTarget.x; targetY=zoneTarget.y;
            action='PRESS'; robot.target=strictlyInsideZone?'BALL_PRESSURE':'ZONE_EDGE_PRESSURE';
          } else if(robot.sweeperState==='RETURN_TO_ZONE'){
            targetX=homeX; targetY=homeY; action='RESET'; robot.target='DEFENSIVE_ZONE_CENTER';
          } else {
            const kickoffHold=this.kickoffRaceTicks>0||this.kickoffFirstKickPending;
            targetX=kickoffHold?robot.homeX:homeX; targetY=kickoffHold?robot.homeY:homeY;
            action='COVER'; robot.target=kickoffHold?'HOME_POST':'DEFENSIVE_ZONE_CENTER';
          }
          ...
          robot.interceptReason=robot.sweeperState==='HOLD_ZONE'?'holding designated home post':robot.interceptReason;
          break;
        }
      }
      ...
      if(!this.kickoffFirstKickPending&&robot.archetype!=='goalkeeper'&&robot.archetype!=='striker'&&!(robot.archetype==='sweeper'&&action==='PRESS')) targetX=this.clamp(targetX+seedBias,28,this.field.width-28);
      ...
      if(robot.archetype!=='striker'){
        for(const other of canonicalRobots){
          ...
          if(dist>0&&dist<64){const push=(64-dist)/dist;const closeBoost=dist<44?4.5:1;targetX+=awayX*push*1.1*closeBoost;targetY+=awayY*push*1.1*closeBoost;}
        }
      ...
      if(robot.archetype==='sweeper'){
        const bounded=this.clampSweeperToZone(robot,targetX,targetY); targetX=bounded.x; targetY=bounded.y;
      }
      ...
      const approachMultiplier=robot.archetype==='goalkeeper'||robot.archetype==='sweeper'?4.5:2.2;
      const approachSpeed=(robot.archetype==='striker'||robot.archetype==='bulwark'||robot.archetype==='sweeper')&&action==='PRESS'?robot.maxSpeed:(closingOnBall?robot.maxSpeed:Math.min(robot.maxSpeed,len*approachMultiplier));
      const strikerMovingAway=robot.archetype==='striker'&&action==='PRESS'&&robot.chargePauseTicks<=0&&velocityTowardTarget<-1;
      const desiredSpeed=robot.archetype==='striker'&&robot.chargePauseTicks>0||strikerMovingAway?0:approachSpeed;
      const desiredX=dx/len*desiredSpeed,desiredY=dy/len*desiredSpeed;
      const maxDelta=robot.acceleration*dt;
      if(strikerMovingAway){robot.vx*=0.25;robot.vy*=0.25;}
      else {robot.vx+=this.clamp(desiredX-robot.vx,-maxDelta,maxDelta);robot.vy+=this.clamp(desiredY-robot.vy,-maxDelta,maxDelta);}
      ...
      if(robot.archetype==='striker'){
        const desiredFacingX=dx/len,desiredFacingY=dy/len;
        if(robot.facingX===0&&robot.facingY===0||robot.facingX*desiredFacingX+robot.facingY*desiredFacingY<0.98){robot.facingX=desiredFacingX;robot.facingY=desiredFacingY;}
      } else if(robot.archetype!=='bulwark'&&robot.archetype!=='sweeper'&&robot.archetype!=='goalkeeper'&&Math.hypot(robot.vx,robot.vy)>1){const facingLen=Math.hypot(robot.vx,robot.vy);robot.facingX=robot.vx/facingLen;robot.facingY=robot.vy/facingLen;}
      ...
      if(robot.archetype==='goalkeeper'){
        if(Math.hypot(robot.x-targetX,robot.y-targetY)<16&&Math.hypot(robot.vx,robot.vy)<100){robot.x=targetX;robot.y=targetY;robot.vx=0;robot.vy=0;}
        robot.x=this.clamp(robot.x,GOAL_AREA.left+ROBOT_RADIUS,GOAL_AREA.right-ROBOT_RADIUS);
        const goalkeeperHomeY=robot.team==='blue'?this.field.height-GOALKEEPER_HOME_DEPTH:GOALKEEPER_HOME_DEPTH;
        const stepOutMin=robot.team==='blue'?goalkeeperHomeY-GOALKEEPER_STEP_OUT:goalkeeperHomeY;
        const stepOutMax=robot.team==='blue'?goalkeeperHomeY:goalkeeperHomeY+GOALKEEPER_STEP_OUT;
        robot.y=this.clamp(robot.y,stepOutMin,stepOutMax);
        robot.moveTargetX=targetX;robot.moveTargetY=targetY;
      }
      if(robot.archetype==='bulwark'||robot.archetype==='sweeper'){if(robot.sweeperState==='INTERCEPT'&&robot.team==='orange'&&robot.y>targetY&&robot.vy>0){robot.y=targetY;robot.vy=0;}if(robot.sweeperState==='INTERCEPT'&&robot.team==='blue'&&robot.y<targetY&&robot.vy<0){robot.y=targetY;robot.vy=0;}}
      ...
      if(robot.archetype==='bulwark'||robot.archetype==='sweeper'){
        const rawX=robot.x,rawY=robot.y; const bounded=robot.archetype==='sweeper'?this.clampSweeperToZone(robot,robot.x,robot.y):this.clampSweeperTarget(robot.team,robot.x,robot.y); robot.x=bounded.x; robot.y=bounded.y;
  ...
  private resolveRobotBallCollisions(){
    ...
    for(const robot of [...this.state.robots].sort((a,b2)=>a.id.localeCompare(b2.id))){
      ...
      if(robot.archetype==='goalkeeper'&&robot.clearCooldown<=0&&!this.kickoffFirstKickPending){
        ...
        this.lastRobotBallContactTick[robot.id]=this.tickIndex; this.ballContactCooldown[robot.id]=8/60;
        continue;
      }
      ...
      if(relative<0){
        ...
        this.lastRobotBallContactTick[robot.id]=this.tickIndex; this.ballContactCooldown[robot.id]=8/60;
        if(!ownGoalRisk&&robot.archetype==='sweeper'&&robot.sweeperState==='INTERCEPT'&&robot.defensiveZone&&b.x>=robot.defensiveZone.left&&b.x<=robot.defensiveZone.right&&b.y>=robot.defensiveZone.top&&b.y<=robot.defensiveZone.bottom&&robot.clearCooldown<=0&&!this.kickoffFirstKickPending) this.applySweeperClear(robot);
      } else {
        if(ownGoalRisk){
      ...
      if(robot.archetype==='striker'){
        robot.chargePauseTicks=61;
        robot.vx=0; robot.vy=0;
        this.ballContactCooldown[robot.id]=1;
      }
  ...
  private applySweeperClear(robot:Robot){
    ...
    robot.clearImpulse=impulse; robot.clearCooldown=0.35; robot.sweeperState='CLEAR'; robot.action='KICK'; robot.kickCooldown=0.45; robot.kickLockout=0.1;
    this.recordEvent({type:'kick',ids:[robot.id],x:b.x,y:b.y,impulse,vyBefore:beforeY,vxBefore:beforeX,vxAfter:b.vx,vyAfter:b.vy,reason:`sweeper clear after physical contact, forward=${attackY.toFixed(3)}`,direction:{x:0,y:attackY},power:impulse,candidate:robot.id,causeContactTick:this.lastRobotBallContactTick[robot.id]});
  }
  ...
  private tryKick(robot:Robot,nx:number,ny:number){
    const b=this.state.ball;
    if((robot.archetype!=='striker'&&robot.archetype!=='cannon')||robot.kickCooldown>0||robot.kickLockout>0||this.ballKickInvuln>0)return;
    const attackY=robot.team==='blue'?-1:1;
    ...
    this.recordEvent({type:'kick',ids:[robot.id],x:b.x,y:b.y,impulse:appliedImpulse,vxBefore:beforeX,vyBefore:beforeY,vxAfter:b.vx,vyAfter:b.vy,reason:robot.lastDecisionReason,direction:{x:appliedDirectionX,y:appliedDirectionY},power:appliedImpulse,candidate:robot.id,causeContactTick:[...this.events].reverse().find((event:SimulationEvent)=>event.type==='robot-ball-collision'&&event.ids?.includes(robot.id))?.tick});
  }

  private integrateBall(dt:number){
    const b=this.state.ball;const wasInsideGoalArea=this.isInsideGoalArea(b.x,b.y);b.x+=b.vx*dt;b.y+=b.vy*dt;
    b.vx*=Math.pow(DAMPING,dt);b.vy*=Math.pow(DAMPING,dt);
    if(!wasInsideGoalArea&&this.isInsideGoalArea(b.x,b.y))this.goalAreaDampingTicks=30;
    if(!this.isInsideGoalArea(b.x,b.y))this.goalAreaDampingTicks=0;
    if(this.goalAreaDampingTicks>0){
      const speed=Math.hypot(b.vx,b.vy);
      if(speed>GOAL_AREA_BALL_SPEED_CAP){b.vx=b.vx/speed*GOAL_AREA_BALL_SPEED_CAP;b.vy=b.vy/speed*GOAL_AREA_BALL_SPEED_CAP;}
      b.vx*=GOAL_AREA_BALL_DAMPING;b.vy*=GOAL_AREA_BALL_DAMPING;
      this.goalAreaDampingTicks--;
    ...
    if(this.sweeperClearTeam&&this.sweeperClearNeedsExit){
      ...
      if(crossedOwnGoalArea){const speedBefore=Math.hypot(b.vx,b.vy);if(speedBefore>SWEEPER_EXIT_SPEED){b.vx=b.vx/speedBefore*SWEEPER_EXIT_SPEED;b.vy=b.vy/speedBefore*SWEEPER_EXIT_SPEED;}const speedAfter=Math.hypot(b.vx,b.vy);this.recordEvent({type:'goal-area-exit-damping',x:b.x,y:b.y,boundary:this.sweeperClearTeam==='blue'?this.field.height-GOAL_AREA.depth:GOAL_AREA.depth,crossingY:b.y,speedBefore,speedAfter,reason:'clear ball crossed own Goal Area boundary'});this.sweeperClearNeedsExit=false;}
    }
 ...
 if(b.y>=this.field.height-18&&b.vy>0&&inGoalMouth){this.state.score.orange++;this.beginGoalReset('orange');return;}
  ...
  private advanceGoalBall(dt:number){const b=this.state.ball;const goalMin=GOAL_LEFT+BALL_RADIUS,goalMax=GOAL_RIGHT-BALL_RADIUS;if(Math.abs(b.vy)<8)b.vy=this.goalTeam==='blue'?-120:120;b.x+=b.vx*dt;b.y+=b.vy*dt;if(b.x<goalMin){b.x=goalMin;if(b.vx<0)b.vx=Math.abs(b.vx)*0.28;}else if(b.x>goalMax){b.x=goalMax;if(b.vx>0)b.vx=-Math.abs(b.vx)*0.28;}b.vx*=Math.pow(0.78,dt);b.vy*=Math.pow(0.78,dt);const netHoldDepth=GOAL_GEOMETRY.depth-BALL_RADIUS;if(b.y<-netHoldDepth){b.y=-netHoldDepth;b.vy=0;}if(b.y>this.field.height+netHoldDepth){b.y=this.field.height+netHoldDepth;b.vy=0;}if(Math.hypot(b.vx,b.vy)<8){b.vx=0;b.vy=0;}}
  private resetBall(){const b=this.state.ball;b.x=this.field.width/2;b.y=this.field.height/2;b.vx=0;b.vy=0;this.goalTeam=undefined;this.safetyGoalPending=undefined;this.lastRobotBallContactTick={};this.sweeperClearTeam=undefined;this.centralDeflectionCooldown=0;this.wallContact={left:false,right:false,top:false,bottom:false};this.resetRobots();this.ballStuckTicks=0;this.lastBallX=b.x;this.lastBallY=b.y;this.state.goalResetTimer=0;this.kickoffTimer=0.75;this.kickoffRaceTicks=180;this.kickoffSafetyTimer=5;this.kickoffFirstKickPending=false;this.recordEvent({type:'kickoff',x:b.x,y:b.y});}
  private resetRobots(){for(const robot of this.state.robots){const formation=this.seedFormation[robot.id];robot.x=formation.x;robot.y=formation.y;robot.vx=0;robot.vy=0;robot.facingX=0;robot.facingY=robot.team==='blue'?-1:1;robot.action='RESET';robot.target='BALL';robot.kickCooldown=0;robot.kickLockout=0;robot.chargePauseTicks=0;robot.sweeperState='HOLD_ZONE';robot.backpedal=false;robot.interceptReason='reset to home post';robot.clearImpulse=0;robot.returnTick=0;robot.clearCooldown=0;}}
  ...
  private decisionReason(robot:Robot,action:Action){
    ...
    if(robot.archetype==='bulwark'||robot.archetype==='sweeper') return robot.sweeperState==='HOLD_ZONE'?'holding designated home post':robot.interceptReason;
  ...
  private recordTelemetry(){
    const frame:TelemetryFrame={tick:this.tickIndex,elapsed:this.state.elapsed,status:this.state.status,goalResetTimer:this.state.goalResetTimer,kickoffSafetyTimer:this.kickoffSafetyTimer,safetyGoalPending:this.safetyGoalPending,score:{...this.state.score},ball:{x:this.state.ball.x,y:this.state.ball.y,vx:this.state.ball.vx,vy:this.state.ball.vy},robots:[...this.state.robots].sort((a,b)=>a.id.localeCompare(b.id)).map(r=>({id:r.id,team:r.team,role:r.role,archetype:r.archetype,x:r.x,y:r.y,vx:r.vx,vy:r.vy,maxSpeed:r.maxSpeed,facingX:r.facingX,facingY:r.facingY,kickCooldown:r.kickCooldown,kickLockout:r.kickLockout,chargePauseTicks:r.chargePauseTicks,target:r.target,action:r.action,moveTargetX:r.moveTargetX,moveTargetY:r.moveTargetY,distanceToBall:r.distanceToBall,distanceToTarget:r.distanceToTarget,sweeperState:r.sweeperState,defensiveZone:r.defensiveZone?{...r.defensiveZone}:undefined,backpedal:r.backpedal,interceptReason:r.interceptReason,clearImpulse:r.clearImpulse,returnTick:r.returnTick,clearCooldown:r.clearCooldown,lastDecisionReason:r.lastDecisionReason,stateChangedAt:r.stateChangedAt,lastKickAt:r.lastKickAt,kickTarget:r.kickTarget,kickDirectionX:r.kickDirectionX,kickDirectionY:r.kickDirectionY,kickPower:r.kickPower})),events:this.events.filter(e=>e.tick===this.tickIndex).map(e=>({...e}))};
  ...
  private clampSweeperToZone(robot:Robot,x:number,y:number){
    const zone=robot.defensiveZone??this.defaultSweeperZone(robot.team);
    return {x:this.clamp(x,zone.left+robot.radius,zone.right-robot.radius),y:this.clamp(y,zone.top+robot.radius,zone.bottom-robot.radius)};
  }
  ...
  private clampSweeperRoleTarget(team:Team,x:number,y:number){
    const outsideBoundary=team==='blue'?this.field.height-GOAL_AREA.depth-ROBOT_RADIUS:GOAL_AREA.depth+ROBOT_RADIUS;
    const base=this.clampSweeperTarget(team,x,y);
    return {x:base.x,y:team==='blue'?Math.min(base.y,outsideBoundary):Math.max(base.y,outsideBoundary)};
  }
  ...
  private isInsideGoalArea(x:number,y:number){return x>=GOAL_AREA.left&&x<=GOAL_AREA.right&&(y<=GOAL_AREA.depth||y>=this.field.height-GOAL_AREA.depth);}
  private isInsideOwnGoalArea(team:Team,x:number,y:number){return x>=GOAL_AREA.left&&x<=GOAL_AREA.right&&(team==='orange'?y<=GOAL_AREA.depth:y>=this.field.height-GOAL_AREA.depth);}
  private clamp(value:number,min:number,max:number){return Math.max(min,Math.min(max,value));}
  snapshot(){ return JSON.parse(JSON.stringify(this.state)); }
  checkpoint(){ return JSON.parse(JSON.stringify({seed:this.seed,seedFormation:this.seedFormation,state:this.state,paused:this.paused,accumulator:this.accumulator,tickIndex:this.tickIndex,kickoffTimer:this.kickoffTimer,kickoffRaceTicks:this.kickoffRaceTicks,kickoffSafetyTimer:this.kickoffSafetyTimer,safetyGoalPending:this.safetyGoalPending,initialKickoffSafety:this.initialKickoffSafety,kickoffPreferredTeam:this.kickoffPreferredTeam,kickoffFirstKickPending:this.kickoffFirstKickPending,ballKickInvuln:this.ballKickInvuln,ballContactCooldown:this.ballContactCooldown,robotCollisionCooldown:this.robotCollisionCooldown,lastRobotBallContactTick:this.lastRobotBallContactTick,ballStuckTicks:this.ballStuckTicks,cornerStuckTicks:this.cornerStuckTicks,sideWallStuckTicks:this.sideWallStuckTicks,sideWallRecoveryLatched:this.sideWallRecoveryLatched,stuckRecoveryCooldown:this.stuckRecoveryCooldown,cornerRecoveryCooldown:this.cornerRecoveryCooldown,centralDeflectionCooldown:this.centralDeflectionCooldown,goalTeam:this.goalTeam,sweeperClearTeam:this.sweeperClearTeam,goalAreaDampingTicks:this.goalAreaDampingTicks,wallContact:this.wallContact,lastBallX:this.lastBallX,lastBallY:this.lastBallY,lastKickTeam:this.lastKickTeam,lastKickX:this.lastKickX,lastKickY:this.lastKickY,lastKickElapsed:this.lastKickElapsed,kickBurstCount:this.kickBurstCount,kickBurstStart:this.kickBurstStart,sweeperClearNeedsExit:this.sweeperClearNeedsExit,events:this.events,telemetry:this.telemetry})); }
