...
describe('responsive portrait presentation contract', () => {
  ...
  it('uses the same 160px width for the goal frame, bar, and goal mouth', () => {
    expect(scene).toContain('g.moveTo(this.field.x+190,this.field.y+80)');
    expect(scene).toContain('g.lineTo(this.field.x+350,this.field.y)');
    expect(scene).toContain('this.add.rectangle(this.field.x+270,this.field.y-9,160,10');
